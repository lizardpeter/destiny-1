...
name. Every exact placement WorldID/rotation/translation remains attached to its
SEntity owner.
"""
...
def flatten_placements(p):
    rows=[]
    for t in p.get('direct_tables',[]):
        rows.extend(t.get('entries',[]))
...
def main():
    ...
    for row in flatten_placements(p):
        h=norm(row.get('entity_hash','FFFFFFFF'))
        if h not in NULLS:by_entity[h].append(row)
    candidates=[];families=defaultdict(lambda:{'entities':set(),'models':set(),'skeleton_resources':set(),'runtime_rig_resources':set(),'placements':0})
    missing_placement=[]
    for e in d.get('entities',[]):
        ...
        for x in by_entity.get(eh,[]):
            placements.append({'source_kind':x.get('source_kind'),'source_hash':x.get('source_hash'),'source_index':x.get('index'),
                               'world_id':x.get('world_id'),'world_id_hex':x.get('world_id_hex'),
        ...
        row={'entity':eh,'classification':cls,'npc_semantic_proven':False,'model_parent_resources':sorted(set(model_parents)),
             ...
             'name_candidates':names,'placement_count':len(placements),'placements':placements,'family_key':family_key,
             'geometry_export_ready':bool(models),'rig_export_candidate':bool(models and skeletons and rigs)}
        candidates.append(row)
        f=families[family_key];f['entities'].add(eh);f['models'].update(models);f['skeleton_resources'].update(x['resource_hash'] for x in skeletons);f['runtime_rig_resources'].update(rigs);f['placements']+=len(placements)
    fam_rows=[]
    ...
    out={'schema_version':1,'status':'D1_WORLD_ARTICULATED_ENTITY_PLAN_COMPLETE' if not missing_placement else 'D1_WORLD_ARTICULATED_ENTITY_PLAN_PARTIAL',
         'candidate_count':len(candidates),'rig_export_candidate_count':sum(c['rig_export_candidate'] for c in candidates),
         'family_count':len(fam_rows),'placement_count':sum(c['placement_count'] for c in candidates),
         ...
         'policy':'Candidate membership comes only from model+skeleton evidence in the source-owned entity dependency graph. Retail names are attached as StringHash evidence/candidates but do not by themselves assign NPC/vendor/combatant semantics.'}
    a.out.parent.mkdir(parents=True,exist_ok=True);a.out.write_text(json.dumps(out,indent=2)+'\n')
    print(json.dumps({k:out[k] for k in ('status','candidate_count','rig_export_candidate_count','family_count','placement_count','unique_model_count','unique_models','unique_skeleton_resource_count','unique_runtime_rig_resource_count','missing_placement_entities')},indent=2))
