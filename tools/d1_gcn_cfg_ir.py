...
This is intentionally below semantic decompilation. It preserves instruction order,
control-flow edges, register operands, shader interpolators/exports, native image
resource provenance and EXEC-mask mutation sites without pretending divergent VGPR
values are ordinary scalar SSA.
"""
...
def main():
    ...
    out={'schema_version':1,'status':'D1_GCN_STRUCTURAL_IR_COMPLETE','shader':shader,'instruction_count':len(ins),'basic_block_count':len(bb),'cfg_edge_count':sum(len(x['successors']) for x in bb),'opcode_counts':dict(sorted(Counter(x['opcode'] for x in ins).items())),'interpolator_inputs':attrs,'parameter_exports':params,'exports':exports,'control_flow':control,'image_usage_summary':None if usage is None else {'image_instruction_count':usage['image_instruction_count'],'used_texture_indices':usage['used_texture_indices'],'image_opcodes':usage['image_opcodes'],'unmatched_image_instruction_count':usage['unmatched_image_instruction_count']},'instructions':ins,'basic_blocks':bb,'semantic_boundary':{'register_def_use_classification':'STRUCTURAL_OPERAND_CLASSIFICATION_ONLY','divergent_vgpr_ssa':'NOT_YET_PROMOTED','exec_mask_symbolic_dataflow':'NEXT_GATE','output_expression_lifting':'WITHHELD_UNTIL_EXEC_MASK_DATAFLOW'},'policy':'Instruction encodings/text, CFG edges, operands, attrs/exports and exact Sony image-resource provenance are promoted. Divergent EXEC makes naive VGPR SSA unsound, so this IR deliberately withholds output-expression claims until symbolic EXEC-mask dataflow is implemented.'}
    a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(out,indent=2)+'\n');print(json.dumps({k:out[k] for k in ('status','shader','instruction_count','basic_block_count','cfg_edge_count','interpolator_inputs','parameter_exports','exports','control_flow','image_usage_summary','semantic_boundary')},indent=2));return 0
