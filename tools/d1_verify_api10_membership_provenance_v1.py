...
def main():
    ...
    expected={x['name']:x['sha256'] for x in m.get('inputs',[])}
    if len(expected)!=2: die('input denominator')
    ...
    if sha256(a.membership)!=m['output']['sha256']: die('membership byte hash drift')
    ...
    if d.get('semantic_boundary',{}).get('engine_semantic')!='WITHHELD': die('semantic boundary weakened')
    print(json.dumps({'status':'D1_API10_MEMBERSHIP_PROVENANCE_EXACT','membership_sha256':sha256(a.membership),'program_count':len(ps),'family_count':len(d['families'])},indent=2))
