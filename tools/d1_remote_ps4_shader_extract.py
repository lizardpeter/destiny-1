...
code.  No shader semantics are inferred.
"""
...
def main() -> int:
    ...
    for shader in sorted({norm(x) for x in a.shader}):
        ...
        elif (hm.get('type'), hm.get('subtype')) != (32, 8):
            row['violations'].append(f'header_type_subtype:{hm.get("type")}:{hm.get("subtype")}!=32:8')
            ...
            if footer is None:
                ...
                try:
                    ...
                    if n <= 0 or n > footer or n > len(pb):
                        ...
                        code = pb[:n]
                        cp = a.out_dir / f'PS_{shader}_gcn.bin'; cp.write_bytes(code)
    ...
    out = {
        'schema': 'd1_remote_ps4_shader_extract/v1',
        ...
        'policy': 'Shader headers and referenced native payloads are exact retail FileEntry resources. GCN code bounds come only from validated OrbShdr metadata; no semantic instruction interpretation occurs here.'
    }
    a.report.parent.mkdir(parents=True, exist_ok=True)
    a.report.write_text(json.dumps(out, indent=2) + '\n')
    print('STATUS', out['status'], 'SHADERS', len(rows), 'VIOLATIONS', len(violations))
    for r in rows:
        print(r['shader'], r.get('native_payload_hash'), (r.get('code') or {}).get('bytes'), (r.get('code') or {}).get('sha256'))
