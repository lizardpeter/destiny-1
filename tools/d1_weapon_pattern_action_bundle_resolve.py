...
This resolver performs that discovery for every supplied weapon pattern. It uses
FileHash package/id routing and exact entry references only. No adjacency or
weapon-type inference is used.
"""
...
def load_member_catalog(path: Path) -> dict[int, list]:
...
def merge_member_catalogs(paths: list[Path], needed: set[int] | None = None) -> dict[int, list]:
    merged: dict[int, list] = {}
    for path in paths:
        cur = load_member_catalog(path)
        for pkg, specs in cur.items():
            if pkg in merged:
                old={(m.name,m.data_offset,m.size) for m in merged[pkg]}
                new={(m.name,m.data_offset,m.size) for m in specs}
                if old != new:
                    raise ValueError(f"conflicting member catalogs for package {pkg:04X}: {path}")
            else:
                merged[pkg]=specs
    if needed is not None:
        missing=sorted(needed-set(merged))
        if missing:
            raise ValueError(f"member catalogs missing package ids {[f'{x:04X}' for x in missing]}")
        merged={pkg:merged[pkg] for pkg in sorted(needed)}
    return merged
...
def main() -> int:
    ...
    ap.add_argument("--investment-member-catalog", action="append", type=Path, required=True,
                    help="validated investment-assets member catalog; repeatable")
    ...
    inv_specs = merge_member_catalogs(args.investment_member_catalog, pattern_pkgs)
    global_specs = merge_member_catalogs([args.globals_member_catalog])
    ...
    out = {
        ...
        "member_catalogs": {
            "investment": [str(x) for x in args.investment_member_catalog],
            "globals": str(args.globals_member_catalog),
        },
