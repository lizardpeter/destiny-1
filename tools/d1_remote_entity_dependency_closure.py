...
- TYPED_EXACT: source/schema-closed serialized relationships.
- UNTYPED_LITERAL: aligned u32 values that exactly resolve to current FileHashes.
  These are discovery evidence only and never semantic ownership by themselves.
"""
...
NO_RECURSE_REFS = {"8080058A", "80800000"}


def filehash_parts(v: int):
...
def printable_runs(b: bytes, min_len: int = 4, max_len: int = 256):
...
def main() -> int:
    ...
    def aligned_resolved(payload: bytes, subject: str):
        found = []
    ...
    q = deque((h, 0, 0) for h in seeds)
    best = {h: (0, 0) for h in seeds}
    nodes = {}
    edges = []
    edge_keys = set()
    violations = []

    def add_edge(subject, predicate, obj, evidence_class, offset=None, attrs=None):
    ...
    def enqueue(target, depth, quality, evidence_class, target_ref):
        nd = depth + 1
        if nd > a.max_depth:
            return
        nq = max(quality, 1 if evidence_class == "UNTYPED_LITERAL" else 0)
        old = best.get(target)
        if old is not None and not (nq < old[0] or (nq == old[0] and nd < old[1])):
            return
        best[target] = (nq, nd)
        if target_ref not in NO_RECURSE_REFS:
            q.append((target, nd, nq))

    while q and len(nodes) < a.max_nodes:
        h, depth, quality = q.popleft()
        if best.get(h) != (quality, depth):
            continue
        if h in nodes and nodes[h].get("processed_quality", 99) <= quality:
            continue
        ...
        row.update(
            {
                "depth": depth,
                "path_class": "TYPED_EXACT_PATH" if quality == 0 else "CONTAINS_UNTYPED_DISCOVERY",
                "processed_quality": quality,
                "violations": row.get("violations", []),
            }
        )
        if h in seeds and depth == 0:
            row["path_class"] = "TYPED_EXACT_ROOT"
            ...
            if ref == S_ENTITY_REF:
                ...
                for rr in resources:
                    ...
                    try:
                        ...
                        enqueue(rh, depth, quality, "TYPED_EXACT", rref)
                                ...
                                try:
                                    ...
                                    for c in ch.get("children", []):
                                        ...
                                        try:
                                            ...
                                            enqueue(eh, depth + 1, quality, "TYPED_EXACT", ce["reference"].upper())
                                        except Exception:
                                            pass
                                except Exception as ex:
                                    rrout["entity_children_parse_error"] = repr(ex)
                ...
                for pred, th in typed_targets:
                    ...
                    try:
                        ...
                        enqueue(th, depth, quality, "TYPED_EXACT", te["reference"].upper())
                    except Exception:
                        pass
                ...
                for m in raw:
                    ...
                    if a.recurse_untyped:
                        enqueue(th, depth, quality, "UNTYPED_LITERAL", tref)
    ...
    out = {
        ...
        "status": "D1_ENTITY_DEPENDENCY_CLOSURE_COMPLETE"
        if not violations
        else "D1_ENTITY_DEPENDENCY_CLOSURE_WITH_VIOLATIONS",
        "seeds": seeds,
        ...
        "proof_policy": {
            "typed_exact": "May support ownership/structure claims when the source-pinned schema says so.",
            "untyped_literal": "Exact current FileHash equality only; discovery evidence, never semantic ownership by itself.",
            "path_class": "Nodes reached through an untyped discovery edge cannot be promoted through that path until the intervening schema is closed.",
        },
    }
    out["truncated"] = bool(len(nodes) >= a.max_nodes and q)
    if out["truncated"]:
    ...
    print(
        ...
        out["edge_count"],
        "TYPED",
        out["typed_edge_count"],
        "UNTYPED",
        out["untyped_literal_edge_count"],
        "ARTICULATED",
        len(out["articulated_entities"]),
        "TRUNCATED",
        out["truncated"],
    )
