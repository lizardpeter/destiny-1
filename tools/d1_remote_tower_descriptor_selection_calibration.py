...
def parse_graph(c,resource_hash):
 ...
 sw=exact[0];switch_count=int(sw['outer_count'])
 # inner_pair_rows is intentionally sparse: the structural probe emits rows only
 # for outer switch records whose nested pair array is non-empty.  Empty switch
 # records are valid serialized records and must remain present as [] rather than
 # being misclassified as missing coverage.
 records={i:[] for i in range(switch_count)}
 seen=set()
 for x in sw.get('inner_pair_rows',[]):
  oi=int(x['outer_index'])
  if oi<0 or oi>=switch_count:raise ValueError(f'{h}: switch row index {oi} OOB')
  if oi in seen:raise ValueError(f'{h}: duplicate switch row index {oi}')
  seen.add(oi);records[oi]=x['pairs']
 ...
 return {'resource_hash':h,'model_tag_hash':norm(p.get('embedded_model_tag_hash','FFFFFFFF')),'sha256':hashlib.sha256(b).hexdigest(),'source':src,'switch_record_count':switch_count,'switch_records':records,'nonempty_switch_record_count':len(seen),'empty_switch_record_count':switch_count-len(seen),'descriptor_count':len(drows),'descriptor_rows':drows,'group_count':len(groups),'groups':groups,'material_count':len(tags),'B_invariant_count':sum(x['B_same_keys_all_invalid'] for x in drows)}
