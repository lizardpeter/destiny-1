...
def logical_package_id_from_path(path:Path)->int:
    m=re.search(r'_([0-9A-Fa-f]{4})_\d+\.pkg    m=re.match(r'^(.*)_\d+\.pkg$',seed.name)
    if not m:
...
def main():
    ...
    ap.add_argument('--dependency-pkg',type=Path,action='append',default=[],
                    help='additional logical package family seed required by serialized GPU FileHashes')
    ...
    for p in a.dependency_pkg:
        pid=logical_package_id_from_path(p)
        if pid in reader_sets:
            raise ValueError(f'duplicate logical package family {pid:04X}: {p}')
        reader_sets[pid]=reader_family(p,a.runtime)
        dependency_package_ids.append(pid)
...
,path.name)
    if not m:
        raise ValueError(f'cannot derive logical package id from {path.name}')
