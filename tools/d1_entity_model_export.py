...
def export_model(r, tag_hash, out_glb, out_json=None, unique_ranges=True):
    ...
    for mi,mesh in enumerate(model['meshes']):
        ...
        s0=hdr_stride(v1h); s1=hdr_stride(v2h)
        ...
        inds=decode_indices(idata,index_is32(ih))
        ranges={}
        ...
        for gi,(key,candidates) in enumerate(ranges.items()):
            ...
            if len(sl)!=count or count%3: raise ValueError(f'mesh {mi} group {gi}: invalid triangle index range')
            if len(sl)==0: continue
            if sl.max()>=len(pos): raise ValueError(f'mesh {mi} group {gi}: index out of vertex range')
            used,inv=np.unique(sl,return_inverse=True); faces=inv.reshape((-1,3))
            ...
            tm.metadata={'model_tag_hash':th,'mesh_index':mi,'source_vertex_indices':used.tolist(),'candidate_materials':materials,'part_indices':[pi for pi,_ in candidates],'lod_values':sorted(set(p['lod'] for _,p in candidates)),'index_offset':off,'index_count':count}
            scene.add_geometry(tm,geom_name=name,node_name=name)
            mrep['primitive_groups'].append({'name':name,'index_offset':off,'index_count':count,'triangle_count':count//3,'unique_vertex_count':len(used),'candidate_materials':materials,'part_indices':[pi for pi,_ in candidates],'lod_values':sorted(set(p['lod'] for _,p in candidates))})
        report['meshes'].append(mrep)
    out_glb.parent.mkdir(parents=True,exist_ok=True); scene.export(out_glb)
    report['glb']=str(out_glb); report['geometry_count']=len(scene.geometry); report['bounds']=scene.bounds.tolist() if scene.bounds is not None else None
