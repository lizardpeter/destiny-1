...
def main()->int:
    ...
    out={'schema_version':1,'status':'D1_TOWER_80C88434_RESOURCE_CLASS_CENSUS_EXACT' if not violations else 'D1_TOWER_80C88434_RESOURCE_CLASS_CENSUS_VIOLATIONS',
         ...
         'policy':'ResourcePointer classes come only from exact relative-pointer targets in retail EntityResource payloads. Prefix windows are discovery evidence for source-schema correlation; no unknown class is semantically named by this probe.'}
    a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(out,indent=2)+'\n')
    print(json.dumps({'status':out['status'],'unique_resource_count':len(unique_rows),'semantic_role_counts':dict(role_counts),'pairs':out['discriminator_parent_pair_counts'],'violations':violations},indent=2))
