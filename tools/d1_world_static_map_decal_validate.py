...
The layout is pinned to MontagueM/Charm commit
...
selected 808008B4 resource. `validate_static_map()` is reusable by generic world-layer
census/export planning tools; the CLI is only a thin adapter around that function.
"""
...
def validate_static_map(c: Corpus, static_map_data: str) -> dict:
    h = norm(static_map_data)
    meta = c.entry_meta(h)
    rep = {
        'schema_version': 2,
...
def main() -> int:
    ...
    rep = validate_static_map(c, a.static_map_data)
    a.out.parent.mkdir(parents=True, exist_ok=True)
    a.out.write_text(json.dumps(rep, indent=2) + '\n')
    print(json.dumps({
        'static_map_data': rep['static_map_data'],
