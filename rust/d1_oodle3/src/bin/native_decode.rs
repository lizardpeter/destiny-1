...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let input = PathBuf::from(args.next().ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "missing input"))?);
    let raw_len_os = args.next().ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "missing raw length"))?;
    let raw_len = parse_usize(&raw_len_os.to_string_lossy())?;
    let output = PathBuf::from(args.next().ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "missing output"))?);
