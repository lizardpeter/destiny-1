...
fn copy_match_into(
    ...
    if distance == 2 {
        unsafe {
            ...
            let src = base.add(*output_pos - 2);
            let a = *src;
            let b = *src.add(1);
            let pattern = u64::from_ne_bytes([a, b, a, b, a, b, a, b]);
            let mut dst = base.add(*output_pos);
            let mut remaining = length;
            while remaining >= 8 {
                core::ptr::write_unaligned(dst.cast::<u64>(), pattern);
                dst = dst.add(8);
                remaining -= 8;
            }
            for i in 0..remaining {
                *dst.add(i) = if (i & 1) == 0 { a } else { b };
            }
        }
        *output_pos += length;
        return Ok(());
    ...
    if distance == 3 {
        unsafe {
            let base = output.as_mut_ptr();
            let src = base.add(*output_pos - 3);
            let a = *src;
            let b = *src.add(1);
            let c = *src.add(2);
            let p0 = u64::from_ne_bytes([a, b, c, a, b, c, a, b]);
            let p1 = u64::from_ne_bytes([c, a, b, c, a, b, c, a]);
            let p2 = u64::from_ne_bytes([b, c, a, b, c, a, b, c]);
            let mut dst = base.add(*output_pos);
            let mut remaining = length;
            while remaining >= 24 {
                core::ptr::write_unaligned(dst.cast::<u64>(), p0);
                core::ptr::write_unaligned(dst.add(8).cast::<u64>(), p1);
                core::ptr::write_unaligned(dst.add(16).cast::<u64>(), p2);
                dst = dst.add(24);
                remaining -= 24;
            }
            let tail_start = length - remaining;
            for i in 0..remaining {
                *dst.add(i) = match (tail_start + i) % 3 {
                    0 => a,
                    1 => b,
                    _ => c,
                };
    ...
    if distance == 4 {
        unsafe {
            let base = output.as_mut_ptr();
            let src = base.add(*output_pos - 4);
            let a = *src;
            let b = *src.add(1);
            let c = *src.add(2);
            let d = *src.add(3);
            let pattern = u64::from_ne_bytes([a, b, c, d, a, b, c, d]);
            let mut dst = base.add(*output_pos);
            let mut remaining = length;
            while remaining >= 8 {
                core::ptr::write_unaligned(dst.cast::<u64>(), pattern);
